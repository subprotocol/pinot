...
public class ReplicaGroupRebalanceSegmentStrategy implements RebalanceSegmentStrategy {
  ...
  public PartitionAssignment rebalancePartitionAssignment(IdealState idealState, TableConfig tableConfig,
    ...
    boolean replicaGroupBootstrap = rebalanceUserConfig.getBoolean(RebalanceUserConfigConstants.REPLICA_GROUP_BOOTSTRAP,
        RebalanceUserConfigConstants.DEFAULT_REPLICA_GROUP_BOOTSTRAP);

    // Compute new replica group partition assignment
    ReplicaGroupPartitionAssignment newPartitionAssignment =
        computeNewReplicaGroupMapping(tableConfig, partitionAssignmentGenerator, replicaGroupBootstrap);
   ...
   * @param replicaGroupBootstrap bootstrap mode
  ...
  private ReplicaGroupPartitionAssignment computeNewReplicaGroupMapping(TableConfig tableConfig,
      ReplicaGroupPartitionAssignmentGenerator partitionAssignmentGenerator, boolean replicaGroupBootstrap) {
    ...
    if (replicaGroupConfig.getPartitionColumn() != null) {
      throw new UnsupportedOperationException("Partition level replica group rebalance is not supported");
    }
    ...
    // If bootstrap mode is enabled, build the replica group partition assignment without checking rebalance mode.
    if (replicaGroupBootstrap) {
    ...
    for (int partitionId = 0; partitionId < oldReplicaGroupPartitionAssignment.getNumPartitions(); partitionId++) {
      for (int groupId = 0; groupId < oldNumReplicaGroup; groupId++) {
        ...
        switch (rebalanceType) {
          ...
          default:
            throw new UnsupportedOperationException(
                "Not supported operation. Please check machine tags and table config. "
                    + "Also, please consider to use BOOTSTRAP mode.");
        }
        if (!removeGroup) {
  ...
  private ReplicaGroupRebalanceType computeRebalanceType(int oldNumInstancesPerPartition, int oldNumReplicaGroup,
    ...
    LOGGER.info("Not supported replica group rebalance type. Need to check server tags and replica group config to"
        + " make sure only one maintenance step is asked. ( oldNumInstancesPerPatition: " + oldNumInstancesPerPartition
        + ", targetNumInstancesPerPartition: " + targetNumInstancesPerPartition + ", oldNumReplicaGroup: "
        + oldNumReplicaGroup + ", targetNumReplicaGroup: " + targetNumReplicaGroup + ", numAddedServers: "
  ...
  private IdealState rebalanceSegments(IdealState idealState, TableConfig tableConfig,
    ...
    // If the table is not using replica group segment assignment strategy or it is based on partition level
    // replica group, we cannot perform the rebalance algorithm
    ReplicaGroupStrategyConfig replicaGroupConfig = tableConfig.getValidationConfig().getReplicaGroupStrategyConfig();
    ...
    // For now, we don't support the rebalance for partition level replica group so "numPartitions" will be 1.
  ...
  private void rebalanceReplicaGroup(List<String> serversInReplicaGroup,
    ...
    // Compute the number of segments per server after rebalance
    int numSegmentsPerServer = segmentsToCover.size() / serversInReplicaGroup.size();

    for (String server : serversInReplicaGroup) {
      // For each server, we either add or remove segments to rebalance
      ...
      // Do nothing if the number of segments to move is zero
    }
